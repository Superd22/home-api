...
import {
  ISynthAfterCompose,
  SynthAfterCompose,
} from '../metadatas/after-compose.decorator';
...
export class AuthMiddlewares implements ISynthAfterCompose {
  ...
  protected readonly logger = new Logger('AuthMiddlewares');
  ...
  public constructor(
    ...
    this.logger.debug('creating auth middleware factory');
  ...
  public registerRule(rule: AuthRule) {
    this.logger.debug('Registering middleware ' + rule.id);
    this.requiredAuths.set(rule.id, rule);
  }

  public synth(): Compose {
    const middlewareCompose = new Compose(this.app, 'auth-middlewares', {
      version: '3.6',
      name: null,
    });

    for (const rule of this.requiredAuths.values()) {
      this.logger.debug('Creating middleware ' + rule.id);
  ...
  protected middlewareFactory(compose: Compose, rule: AuthRule): WebService {
    return new WebService(compose, 'forward-auth' + rule.id, {
      serviceProps: {
        ...
        command: keyValueFromConfig({
          ...
          providers: {
            google: {
              'client-id':
                this.appConfig.traefik.auth.providers.google.clientId,
              'client-secret':
                this.appConfig.traefik.auth.providers.google.clientSecret,
            },
            'generic-oauth': {
              ...
              'client-id':
                this.appConfig.traefik.auth.providers.github.clientId,
              'client-secret':
                this.appConfig.traefik.auth.providers.github.clientSecret,
            },
          },
        }).map((keyVal) => {
...
export class AuthRule {
  public constructor(args: { domains: string[] });
  public constructor(args: { whitelist: string[] });
  public constructor(
    public readonly domainsOrWhiteList:
      | { domains: string[] }
      | { whitelist: string[] }
  ) {}

  public get id(): `whitelist-${string}` | `domains-${string}` {
    return `${this.type}-${createHash('md5')
      .update(this.domainsOrWhiteList[this.type].join('-'))
      .digest('hex')}`;
  }
